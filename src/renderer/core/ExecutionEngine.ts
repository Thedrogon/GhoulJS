...
function __loopGuard(){
  if(++__loopCount>=__maxLoops) throw new Error('Infinite loop detected - execution stopped for safety');
  return true;
}
...
`;
  ...
  async run(rawCode: string, sink: LogSink) {
    if (containsDangerousCode(rawCode)) {
      sink("security", ["Potentially dangerous code detected"]);
      return;
    }
      ...
      (acc: any, k) => {
        acc[k] = (...args: any[]) => {
          ...
          if (lines >= MAX_OUTPUT_LINES) {
            sink("security", [
              `Output limit reached (${MAX_OUTPUT_LINES} lines)`,
            ]);
            abortCtrl.abort();
            return;
          }
          if (Date.now() - started > EXECUTION_TIMEOUT) {
            sink("security", ["Execution stopped due to timeout"]);
            abortCtrl.abort();
            return;
          }
    ...
    const timeout = setTimeout(() => {
      if (!abortCtrl.signal.aborted) {
        sink("security", [
          `Execution stopped due to timeout (${EXECUTION_TIMEOUT / 1000}s)`,
        ]);
        abortCtrl.abort();
      }
    }, EXECUTION_TIMEOUT);
  ...
  private formatLoopError(code: string, loc: { type: string; line: number; column: number }): string {
    const msg = loc.type === 'while' 
      ? 'SyntaxError: Infinite loop detected - while loop missing execution body'
      : 'SyntaxError: Incomplete control structure';
